...
import onealldigital.nizara.in.adapter.CheckoutItemListAdapter;
...
//        tvDeliveryCharge = root.findViewById(R.id.tvDeliveryCharge);
        tvDeliveryCharge = root.findViewById(R.id.tv_auto_fare);
...
//                                        tvTotalBeforeTax.setText(session.getData(Constant.CURRENCY) + ApiConfig.StringFormat("" + subtotal));
                                        tvSubTotal.setText("Grand Total : "+session.getData(Constant.CURRENCY)+ApiConfig.StringFormat(String.valueOf(Constant.FLOAT_TOTAL_AMOUNT))/* + ApiConfig.StringFormat("" + (subtotal + dCharge))*/);
...
//                    lytSaveAmount.setVisibility(View.GONE);
            ...
            if (Constant.FLOAT_TOTAL_AMOUNT <= Constant.SETTING_MINIMUM_AMOUNT_FOR_FREE_DELIVERY) {
                ...
                tvDeliveryCharge.setText("Auto Fare : "+getResources().getString(R.string.free));
                deliveryCharge = "0";
            }
            dCharge = tvDeliveryCharge.getText().toString().equals(getString(R.string.free)) ? 0.0 : Constant.SETTING_DELIVERY_CHARGE;
            if (!pCode.isEmpty()) {
...
//            tvTotalBeforeTax.setText(session.getData(Constant.CURRENCY) + ApiConfig.StringFormat("" + subtotal));
//            tvSubTotal.setText("Grand Total : "+session.getData(Constant.CURRENCY)+ApiConfig.StringFormat(String.valueOf(Constant.FLOAT_TOTAL_AMOUNT))/* + ApiConfig.StringFormat("" + (subtotal + dCharge))*/);
            tvSubTotal.setText("Grand Total : "+session.getData(Constant.CURRENCY)+ ApiConfig.StringFormat("" + (subtotal + dCharge))/* + ApiConfig.StringFormat("" + (subtotal + dCharge))*/);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
